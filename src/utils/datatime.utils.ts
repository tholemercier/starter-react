...
export const timeLikeToMs = (time: TimeLike) => {
  ...
  if (time.endsWith('ms')) return +time.slice(0, -2)
  if (time.endsWith('s')) return +time.slice(0, -1) * oneSecond
  if (time.endsWith('m')) return +time.slice(0, -1) * oneMinute
  if (time.endsWith('h')) return +time.slice(0, -1) * oneHour
  if (time.endsWith('d')) return +time.slice(0, -1) * oneDay
  if (time.endsWith('w')) return +time.slice(0, -1) * oneWeek
  if (time.endsWith('mo')) return +time.slice(0, -2) * oneMonth
  if (time.endsWith('y')) return +time.slice(0, -1) * oneYear
  return NaN
}
